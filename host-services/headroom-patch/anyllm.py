...
class AnyLLMBackend(Backend):
    ...
    async def stream_openai_message(
        ...
        try:
            ...
            # Emit an OpenAI-compatible SSE prelude before opening the
            # upstream request. This is intentionally a real chat.completion
            # chunk (empty delta), not an SSE comment: downstream clients parse
            # it as a stream part and disarm first-byte/startup guards while the
            # real upstream is still working. It carries no user-visible content
            # and is safe for clients that ignore empty deltas.
            prelude = {
