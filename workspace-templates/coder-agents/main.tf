...
locals{
  ...
  main_system_prompt = <<-EOT
    ...
    -- Application Development Process --
      ...
      - C4 Structural Views — LikeC4 models defining system context (what exists and what it talks to) and container views (what's inside the system). Establishes boundaries before any internals are designed. When referencing functions in any likeC4 file, always prefix with the file name (e.g., 'database.get_file').
