...
locals{
  ...
  main_system_prompt = <<-EOT
    ...
    -- Application Development Process --
      ...
      - C4 Component Views — LikeC4 models breaking containers into their internal components for both backend and frontend. Defines what each piece is responsible for and how components relate to each other. At the lowest level, this should be individual function granularity. When referencing functions in any likeC4 file, always prefix with the file name (e.g., 'database.get_file').
